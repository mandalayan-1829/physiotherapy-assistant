...
def register(payload: RegisterRequest, db: DbSession) -> TokenResponse:
    """Create an account (patient or doctor) and return an access token."""
    account = auth_service.register_account(db, payload)
...
def login(payload: LoginRequest, db: DbSession) -> TokenResponse:
    ...
    account = auth_service.authenticate(db, payload.email, payload.password, payload.role)
    token, expires_in = auth_service.issue_token(account)
    return TokenResponse(
        access_token=token,
        expires_in=expires_in,
...
def forgot_password(payload: ForgotPasswordRequest, db: DbSession) -> ForgotPasswordResponse:
    ...
    account, in either portal.
    """
    message = password_reset_service.request_password_reset(db, payload.email, payload.role)
    return ForgotPasswordResponse(message=message)


@router.post("/verify-reset-code", response_model=VerifyResetCodeResponse)
def verify_reset_code(
    payload: VerifyResetCodeRequest, db: DbSession
) -> VerifyResetCodeResponse:
    ...
    reset_token, expires_in = password_reset_service.verify_reset_code(
        db, payload.email, payload.verification_code, payload.role
    )
